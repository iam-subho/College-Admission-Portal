...
const blank = () => ({
    ...
    mode: 'stub',
    priority: 100,
    config: { api_key: '', source_number: '', app_name: '' },
});
...
const startEdit = (p) => {
    ...
    form.config = { api_key: '', source_number: '', app_name: '' };
    showForm.value = true;
};
...
const submit = () => {
    ...
    if (editingId.value) {
        router.patch(route('admin.whatsapp-providers.update', editingId.value), form, opts);
    } else {
        router.post(route('admin.whatsapp-providers.store'), form, opts);
    }
};

const toggle = (p) => router.post(route('admin.whatsapp-providers.toggle', p.id), {}, { preserveScroll: true });
const destroy = (p) => {
    if (!confirm(`Delete provider "${p.display_name}"?`)) return;
    router.delete(route('admin.whatsapp-providers.destroy', p.id), { preserveScroll: true });
};
</script>

<template>
    <Head title="WhatsApp Providers" />
    <PortalLayout title="WhatsApp Providers" :breadcrumb="['Admin', 'WhatsApp Providers']">

        <p class="text-sm text-ink-mute mb-3">
            WhatsApp Business API providers. Only one can be active at a time. Each event needs a
            <strong>pre-approved template name</strong> (configured per-template in /admin/notification-templates).
            Stub mode logs without sending.
        </p>
        ...
        <div v-if="showForm" class="bg-white border border-border rounded p-4 mb-6">
            <h3 class="font-serif text-lg text-maroon mb-3">
                {{ editingId ? `Edit Provider #${editingId}` : 'New WhatsApp Provider' }}
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <InputText v-model="form.config.api_key" label="API Key" placeholder="Leave blank to keep existing" />
                <InputText v-model="form.config.source_number" label="Source Number (E.164)" placeholder="+919999988888" />
                <InputText v-model="form.config.app_name" label="App Name (src.name)" />
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr v-if="!providers.length">
                        <td colspan="7" class="px-4 py-6 text-center text-ink-mute text-sm">No providers yet. Add Gupshup above with stub mode for testing.</td>
                    </tr>
                </tbody>
            </table>
        </div>
